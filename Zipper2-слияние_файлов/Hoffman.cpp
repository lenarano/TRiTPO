...
void Hoffman::unzipping(std::map<char,int> &counting,Package *zip)
{ 
	int pos,n;
	std::string str1=zip->hoff_bin_file.substr(0, 8);
	std::string str2=zip->hoff_bin_file.substr(15, 16);
	std::string hoff_bin= str1+str2;
	std::ifstream F(hoff_bin, std::ios::in | std::ios::binary);
	int sz;
	...
	std::ofstream O(zip->unhoff_bin_file, std::ios::out | std::ios::binary);
	Node *pp = root;
	count=0;
	char byte;
	byte = F.get();
