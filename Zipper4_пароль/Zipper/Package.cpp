...
using namespace std;
Package::Package(std::vector<std::string> &vec, std::string p,std::string n,std::string par)
{
...
std::string Package::getPassword(){return password;}
...
void Package::unPacking1(string binary)
{
	...
	info_block = new char[_sz];  // информационный блок 
	fread(info_block,1,_sz,bin);   // считываем его 
	// Парсинг информационного блока :
	tok = strtok(info_block,"||");
	password=tok;
	tok=strtok(NULL,"||");
}

void Package::unPacking2(string binary)
{
