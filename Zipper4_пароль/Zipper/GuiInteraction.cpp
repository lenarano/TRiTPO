	...
	{			
		...
		else
		{
			...
			else
			{
				...
				zip = new Package(files,path,dialogNameFile.fileName);
				zip->packaging();//óïàêîâêà ôàéëîâ â îäèí
				Íaff.archiving(zip,Wood,Íaff);
...
{		
	if(ifFileUnarchive)
	{
		...
		else
		{
			...
			Íaff.unÑompression(Wood.getCounting(),files,zip);//ðàçàðõèâàöèÿ
