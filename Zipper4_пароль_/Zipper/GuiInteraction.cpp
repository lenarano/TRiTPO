...
void GuiInteraction::cleaning()
{
	...
	path = "";
	dialogNameFile.fileName ="";
	dialogNameFile.name ="";
	dialogNameFile.parole ="";
	zip->password="";
}
	...
	{			
		...
		else
		{
			...
			else
			{
				...
				zip = new Package(files,path,dialogNameFile.fileName,dialogNameFile.parole);
...
{		
	if(ifFileUnarchive)
	{
		...
		else
		{
			...
			if(zip->password!="нет_пароля") dialogNameFile.PasswordVerification(zip->password);
			if(dialogNameFile.check==true||zip->password=="нет_пароля")
			{
